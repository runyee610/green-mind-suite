    ...
    <div className="px-4 py-4">
      ...
      <div className="flex flex-wrap items-start gap-3">
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          ...
          {(() => {
            const w = parseFloat(String(parent.weight ?? ""));
            const hasWeight = Number.isFinite(w) && w > 0;
            const scored = rows.filter((r) => typeof r.aiMeta?.score === "number");
            const earned = hasWeight && scored.length > 0
              ? Math.round(
                  scored.reduce((s, r) => {
                    const rw = parseFloat(String(r.weight ?? ""));
                    return s + ((r.aiMeta!.score / 100) * (Number.isFinite(rw) ? rw : 0));
                  }, 0) * 10,
                ) / 10
              : null;
            return (
              <Badge variant="outline" className="border-primary/40 bg-primary/5 text-xs text-primary">
                {earned !== null ? (
                  <>
                    得分 <span className="mx-0.5 font-mono font-semibold">{earned}</span>
                    <span className="text-primary/60"> / 分值 {parent.weight}</span>
                  </>
                ) : (
                  <>分值 {parent.weight ?? "/"}</>
                )}
