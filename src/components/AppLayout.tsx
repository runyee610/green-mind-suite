...
const ROUTE_TITLES: Array<{ match: RegExp; crumbs: string[] }> = [
  ...
  { match: /^\/green-mfg\/(gov|ent)\/declaration\/new/, crumbs: ["绿色制造", "模拟评价", "开始评价"] },
  { match: /^\/green-mfg\/(gov|ent)\/declaration\/[^/]+/, crumbs: ["绿色制造", "模拟评价", "详情"] },
  { match: /^\/green-mfg\/(gov|ent)$/, crumbs: ["绿色制造", "模拟评价"] },
  { match: /^\/green-mfg-agent/, crumbs: ["绿色制造", "数据智能"] },
  { match: /^\/policy-agent/, crumbs: ["绿色制造", "政策推送"] },
  { match: /^\/report-monthly\/filling/, crumbs: ["经信委节能月报", "填报"] },
  { match: /^\/report-monthly/, crumbs: ["经信委节能月报"] },
  ...
  { match: /^\/system\/users/, crumbs: ["系统管理", "账号管理"] },
  { match: /^\/system\/permissions/, crumbs: ["系统管理", "权限配置中心"] },
  { match: /^\/system/, crumbs: ["系统管理"] },
  { match: /^\/direct-benefit\/gov\/policies\/[^/]+/, crumbs: ["免审即享", "政策图谱", "详情"] },
  { match: /^\/direct-benefit\/(gov|ent)\/policies/, crumbs: ["免审即享", "政策图谱"] },
  { match: /^\/direct-benefit\/gov\/entprofile\/[^/]+/, crumbs: ["免审即享", "企业画像", "详情"] },
  { match: /^\/direct-benefit\/gov\/entprofile/, crumbs: ["免审即享", "企业画像"] },
  { match: /^\/direct-benefit\/gov\/matches\/[^/]+/, crumbs: ["免审即享", "撮合名单", "详情"] },
  { match: /^\/direct-benefit\/gov\/matches/, crumbs: ["免审即享", "撮合名单"] },
  { match: /^\/direct-benefit\/gov\/sources/, crumbs: ["免审即享", "数据源配置"] },
