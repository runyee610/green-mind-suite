...
import { AlertTriangle, Boxes, Download, Eye, FileSpreadsheet, FilterX, Link2, Search, Upload, Users } from "lucide-react";
...
export default function Assets() {
  const [selected, setSelected] = useState<InvestmentProject>(projects[0]);
  ...
  const stats = useMemo(() => {
    ...
    const totalApproved = projects.reduce((s, p) => s + p.approvedEnergy, 0);
    const pendingLink = projects.filter((p) => p.linkStatus !== "已关联").length;
    return [
      { label: "项目总数", value: total, hint: "本年度普通固定资产投资项目", icon: Boxes, accent: "text-primary" },
      { label: "本年度累计批复能耗", value: `${fmt(totalApproved)} 吨标煤`, hint: "等价值，含全部已立项项目", icon: FileSpreadsheet, accent: "text-secondary" },
      { label: "待关联企业项目", value: pendingLink, hint: "含待关联 / 待确认", icon: Link2, accent: "text-warning" },
    ];
  }, []);
  ...
  return (
    <AppLayout title="固定资产管理" subtitle="政府侧普通固定资产投资项目统一视图、检索与企业关联">
      <div className="grid grid-cols-1 gap-4 2xl:grid-cols-[minmax(0,1.05fr)_minmax(620px,0.95fr)]">
              ...
              <Card key={s.label} className="panel">
                <CardContent className="flex items-center justify-between p-4">
                  <div>
                    ...
                    <div className={cn("mt-1.5 font-mono text-2xl font-semibold", s.accent)}>{s.value}</div>
                    <p className="mt-0.5 text-[11px] text-muted-foreground">{s.hint}</p>
                  </div>
                  <div className={cn("flex h-10 w-10 items-center justify-center rounded-lg bg-muted/40", s.accent)}>
                ...
                <Table>
                  ...
                  <TableBody>
                    {filtered.map((p, i) => {
                      ...
                      return (
                        <TableRow key={p.id} data-state={active ? "selected" : undefined} className="cursor-pointer" onClick={() => setSelected(p)}>
                          ...
                          <TableCell>
                            <div className="flex items-center gap-1">
                              <Button size="icon" variant="ghost" className="h-7 w-7" onClick={(e) => { e.stopPropagation(); setSelected(p); }}>
                                <Eye className="h-3.5 w-3.5" />
                              </Button>
                              <Button size="icon" variant="ghost" className="h-7 w-7" onClick={(e) => { e.stopPropagation(); openLink(p); }}>
        ...
        <aside className="min-w-0">
          <ProjectDetailView project={selected} onLink={() => openLink(selected)} />
        </aside>
      </div>

      <ImportDialog open={importOpen} onOpenChange={setImportOpen} />
      <LinkEnterpriseDialog open={linkOpen} onOpenChange={setLinkOpen} project={linkTarget ?? selected} />
    </AppLayout>
  );
}
